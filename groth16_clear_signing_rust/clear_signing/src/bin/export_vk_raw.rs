// Converts the existing ark-serialize verifying key
// (artifacts/clear_signing_transfer_vk.bin) into the raw byte layout the C
// verifier consumes, without re-running setup (the artifact set stays intact):
//
//   vk_raw.bin = IC[0..2] (3 x 64 B G1) || e(alpha,beta) (384 B GT)
//                || -gamma (128 B G2) || -delta (128 B G2)   = 832 B
//
// Encodings match export_artifacts: G1 = BE x||y, G2 = x.c0||x.c1||y.c0||y.c1,
...
fn main() {
    ...
    let vk_path = dir.join("clear_signing_transfer_vk.bin");
