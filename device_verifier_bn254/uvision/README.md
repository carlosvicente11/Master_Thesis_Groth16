...
```sh
cd thesis/device_verifier_bn254
...
`clear_signing_m4` compiles the RELIC-independent `src/fr.c`/`src/poseidon.c`
(portable C99, no `__int128`) and the generated clear-signing VK/vector
headers (`include/vk_clear_signing.h`, `test/clear_signing_vectors.h`) — all
owned by this project; both µVision options below apply to it unchanged
(image: ~63 KB flash, ~19 KB RAM).
