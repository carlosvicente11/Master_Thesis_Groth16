...
void _start(void) {
    ...
    /* 32-bit semihosting SYS_EXIT takes the reason code directly in r1 (not a
     * pointer). ApplicationExit -> QEMU exits 0, anything else -> 1, so the
     * process exit code reflects main()'s success. */
    semihost_call(SYS_EXIT,
                  (void *)(rc == 0 ? 0x20026 : 0x20024));
    while (1) {}
}
...
void abort(void) {
    semihost_call(SYS_EXIT, (void *)0x20024); /* ADP_Stopped_InternalError */
    while (1) {}
}

void exit(int code) {
    semihost_call(SYS_EXIT, (void *)(code == 0 ? 0x20026 : 0x20024));
    while (1) {}
}

int raise(int sig) { (void)sig; abort(); return 0; }

/* RELIC uses RAND=CALL with no callback registered: the pairing path is fully
 * deterministic and never calls rand_bytes (it would throw ERR_NO_RAND if it
 * did). These POSIX stubs exist only to satisfy any residual libc references
 * at link time. */
