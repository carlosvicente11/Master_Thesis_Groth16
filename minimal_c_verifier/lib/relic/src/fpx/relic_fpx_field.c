...
void fp2_field_init(void) {
	...
	RLC_TRY {
		...
		/* Ethereum BN254 (seed 0x44E992B44A6909F1) needs qnr2=9 (xi=9+u)
		 * to match the tower construction used by the Ethereum ecosystem. */
		if (bn_bits(&(ctx->par)) == 63) {
			uint8_t seed[8];
			bn_write_bin(seed, 8, &(ctx->par));
			if (seed[0] == 0x44 && seed[1] == 0xE9 && seed[2] == 0x92 && seed[3] == 0xB4 &&
			    seed[4] == 0x4A && seed[5] == 0x69 && seed[6] == 0x09 && seed[7] == 0xF1) {
				ctx->qnr2 = 9;
			}
		}
#endif /* !FP_QNRES */
